...
detecta qué te falta (Node, el CLI, el binario `darq`), te muestra el plan antes de tocar nada,
...
Termina dejándote siempre en la interfaz de DARQ: si instaló algo nuevo, bajo OpenCode todavía hay
...
avisarte si hay una versión más nueva de darq publicada. [INSTALL.md](INSTALL.md) explica paso a
paso qué hace ese script, sus flags (`--cli`, `--verify`, `--yes`, `--no-run`, `--bin-dir`, ...), la
interfaz interactiva de DARQ y el uso por línea de comandos — incluida la instalación manual, sin
...
`darq` es un solo archivo: no busca un venv, no depende de nada instalado antes que él, no hace
...
DARQ distribuye un payload seleccionado para el CLI elegido, no un home directory de reemplazo:

- comandos para SDD, contexto, handoff, creación y registro de skills;
- el orquestador de DARQ y sus roles de implementación/verificación;
...
una autorización única por fuera de DARQ (bajo OpenCode, `opencode mcp auth jira`) antes de que
...
`darq.adapters.available()`, así que un tercer CLI que se registre sin agregar su fila hace fallar
...
- **`per_agent_model`**: bajo OpenCode, `darq models set` asigna un modelo distinto por agente,
...
Para `darq install --cli <id>` (y para `update`/`uninstall` con ese mismo `--cli`) necesitás ese
CLI ya instalado en la cuenta: `darq` en sí nunca instala OpenCode ni Claude Code, sólo se integra
con una instalación existente. Si instalaste con `install.sh` de la sección anterior, esto ya está
resuelto — ese script instala Node y el CLI elegido antes de llegar a `darq`, para cualquiera de
los dos (Node es incondicional: parte de los MCPs que DARQ distribuye se materializa con npm,
...
separado antes del apply; DARQ no descarga navegadores. Context7 y Jira son remotos: no descargan
nada, pero necesitan salida a la red, y Jira además su autorización única (bajo OpenCode, `opencode
mcp auth jira`), que DARQ no puede hacer por vos.

DARQ no configura credenciales, proveedor ni modelo. Bajo OpenCode, usá `/connect` para las
credenciales del proveedor y `/models` para elegir el modelo; bajo Claude Code, esa configuración es
la de tu propia cuenta y queda fuera de lo que DARQ toca en cualquiera de los dos casos.
...
DARQ se distribuye bajo la [licencia MIT](LICENSE).
