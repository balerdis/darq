...
|-------|-----------------|--------------------------|
| `tests/test_rebrand.py` apunta `ENGINE_CONTENT_ROOT` a `build/work/extracted/pegasus/content`, que después del primer build es la *salida* del build anterior, no la entrada del motor: el transform reescribe en el lugar, y ahí viven `arquitecto-darq.md` y `darq-general.md`, nunca `king-pegasus.md`. Los dos tests que dicen correr el transform contra contenido real del motor están afirmando idempotencia sobre un árbol ya transformado; y en un clon limpio ni eso, porque `build/` está en `.gitignore` y los dos se saltean | Decidir de dónde sale un árbol de motor sin transformar. Hoy no queda ninguno: `build_darq.py` extrae y reescribe encima, sin copia previa. Guardar el extraído crudo antes de transformar es lo que habilitaría el test que la suite cree tener | Leerlos por lo que prueban de verdad —que el transform es idempotente y que el mirror no se queja— y no como cobertura del rebranding contra el motor. La cobertura real de eso la da `tools/check.sh`, que construye e instala de verdad |
...
| `tools/check.sh` no tiene ningún test propio, y es el único que ejercita la mitad de I/O de `build_darq.py`: descarga, verificación de checksums, extracción, overlay y empaquetado. Ningún archivo de `tests/` lo corre; la única mención es una línea de docstring en `tests/test_build_darq.py` | Decidir cómo se testea un script cuyo trabajo es bajar archivos de la red y construir un artefacto real. La corrida completa es la prueba, y hoy se hace a mano | Correrlo. Es el punto de entrada único y documentado, y una corrida limpia es 18 pass, 5 notes, 0 warnings, 0 failures |
