...
the filesystem port's own :meth:`~darq.ports.filesystem.FileSystem.data_dir`
for that location instead of restating it, the same call
:func:`darq.infra.journal_store_file.journal_path` makes, so the two
...
from darq.core import codecs, snapshot as snapshot_module
from darq.core.snapshot import Entry, Manifest, SnapshotError
from darq.core.types import Codec
from darq.infra.journal_store_file import DATA_DIR_MODE, FILE_MODE
from darq.ports.filesystem import FileSystem, FileSystemError
from darq.ports.snapshot_store import Capture, Retention, SnapshotStoreError
...
def capture_paths(
    ...
    than in :mod:`darq.core.snapshot` because reading bytes and a mode off
