...
:mod:`darq.core.snapshot` is pure: it knows what a manifest means, never
...
from darq.core.snapshot import Manifest
...
class Capture:
    ...
    mirroring :class:`darq.core.snapshot.Entry`, whose ``blob`` reference
    the store derives from this once the bytes are written.

    ``is_directory`` mirrors the same field on :class:`~darq.core.snapshot.Entry`
...
class SnapshotStore(Protocol):
    ...
    def save(self, captures: Sequence[Capture], *, taken_at: str, label: str | None = None) -> int:
        ...
        :class:`darq.core.snapshot.Manifest`. Left ``None``, the
