...
Mirrors `darq.core.dependencies`'s own verification discipline: fetch,
...
`os.replace` that makes the swap atomic (see `darq.infra.fs_posix`'s own
...
from darq.core.identity import SAFE_VERSION, ReleaseSource
...
# now *requires* a caller's own `ReleaseSource` (see `darq.core.identity`),
...
def _tag(version: str) -> str:
    ...
    `SAFE_VERSION` charset at parse time (see `darq.core.identity`).
