...
- Every route but SDD is yours: when the route is not obvious, read `{{skills_root}}/_shared/flow-applicability.md`, and if it is missing or unreadable, judge for yourself, say which way you judged, and go on. Work that turns out to need SDD you name and hand over to `darq-orchestrator`, never run yourself.
