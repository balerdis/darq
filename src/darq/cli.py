...
from darq.adapters.opencode import render as opencode_render_module
from darq.adapters.opencode.manifest import CLI_ID as OPENCODE_CLI_ID
from darq.core import catalog as catalog_module
from darq.core import content as content_module
from darq.core import dependencies as dependencies_module
...
from darq.core import ownership, planner, pointer
from darq.core import upgrade as upgrade_module
from darq.core.identity import Identity
from darq.core.journal import KINDS, Install, Record
from darq.core import mcp_handshake
from darq.core.types import Capability, Codec, Environment, ModelAssignment
...
from darq.ports.model_assignment_store import ModelAssignmentStoreError
from darq.ports.npm_installer import NpmInstaller
from darq.ports.snapshot_store import SnapshotStoreError
...
class Runtime:
    ...
    #: back `True`; for `python3 -m darq` (this repo's own tests, and
...
def default_identity() -> Identity:
    ...
    back to, by design (see `darq.core.identity`'s own docstring).
    """
    document = _package_files("darq").joinpath("identity.json").read_bytes()
...
def main(argv: list[str] | None = None, *, runtime: Runtime | None = None) -> int:
    ...
    if arguments.command is None:
        ...
        if _attached_to_a_terminal():
            # Imported here, not at module level: `darq.tui.app` imports
            ...
            # this module ran the moment `darq.cli` itself was loaded.
            try:
                from darq.tui import app as tui_app
...
def _running_binary_path(runtime: Runtime) -> Path | None:
    ...
    `python3 -m darq` (this repo's own test suite, and a `PYTHONPATH=src`
...
def _merged(
    ...
    `darq.__version__`, which names the pinned engine, never the product a
...
def models_apply(
    ...
    `_grant_mcp_write`'s own docstring in `darq.tui.session`). A CLI
