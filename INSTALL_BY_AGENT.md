# Instalar DARQ con un agente

Este instructivo es para un agente que instala DARQ 5 en nombre de una persona, en la cuenta Linux
actual. DARQ soporta hoy dos CLIs anfitrionas -- OpenCode y Claude Code -- y este documento nunca
...
DARQ 5 es un solo archivo: `darq`. No hay wheel, no hay venv, no hay `pip install` — se
...
**Nunca invoques `darq` sin subcomando, bajo ninguna circunstancia.** Corrido en una terminal abre
...
`darq` instalados -- sin elegir MCPs todavía --, `install.sh` hace eso solo, y es más corto que
...
```sh
curl -fsSL https://github.com/balerdis/darq/releases/latest/download/install.sh \
  | bash -s -- --cli <cli> --yes --no-run
```

`--yes` salta la confirmación (necesaria: nadie va a escribir "y" en un pipe) y `--no-run` evita que
el script lance nada al final -- la TUI de DARQ, que no es algo que un agente deba abrir. Repetido
...
el checkout de `darq` — y no importa: instalar DARQ no exige ninguno. No le pidas nada a
...
```sh
DOWNLOAD_DIR="$HOME/Downloads/darq"
...
assert manifest['schema'] == 'darq-release/v5', manifest['schema']
names = {a['name'] for a in manifest['assets']}
assert names == {'darq', 'install.sh'}, names
...
instalar. DARQ no tiene dependencias de terceros, así que ningún paso posterior vuelve a salir a
internet.
...
lo asumas de ningún otro lado.** Reporta cada adapter que DARQ trae registrado, uno por línea, con
...
```
$ darq doctor
Claude Code: present at /home/.claude, DARQ not installed.
OpenCode: present at /home/.config/opencode, DARQ not installed.
```
...
querés que DARQ integre (o si las dos), en vez de elegir vos. Si las dos dicen `not found on this
machine`, no hay nada para integrar todavía: decíselo a la persona antes de seguir, no instales un CLI
por su cuenta -- eso es tarea de la sección anterior (paso 0), no de DARQ.

## Instalar en el CLI elegido y elegir MCPs

Con `darq` en el PATH y ya sabiendo, por el paso 4, cuál CLI vas a integrar (llamalo `<cli>` de
...
| `playwright` | Maneja un navegador real contra una página real. | `npm` (tarball con `integrity` y lockfile propio) | Necesita Node para instalarse (ver "Preflight de Node") y un navegador compatible ya instalado; DARQ no descarga navegadores. |
...
```sh
darq install --cli <cli> --dry-run --mcp context7 --mcp jira
```

Si la persona ya corre uno de estos servidores por su cuenta bajo una clave propia, no lo instales de
nuevo: usá `--mcp <id>=<clave>` (por ejemplo `--mcp cbm=codebase-memory-mcp`). Esa forma pide a DARQ
...
DARQ conoce, con descriptor propio. Si la persona instaló y administra un MCP que DARQ nunca
vio -- Sentry, Figma, o cualquier otro -- ese servidor puede figurar en la configuración propia del
CLI elegido y seguir siendo invisible para todos los agentes: DARQ renderiza cada agente con una
base que niega todo, y un servidor no nombrado en esa base no se abre aunque exista en la
configuración general del CLI.

`darq mcp grant` es el único mecanismo para eso, y es deliberadamente parejo: la clave se otorga a
...
```sh
darq mcp grant --cli <cli> <clave> --json
```
...
```sh
darq mcp list --cli <cli> --json
darq mcp revoke --cli <cli> <clave> --json
```
...
declarada que DARQ ya alcanza por agente -- un servidor propio que esta instalación eligió, o una
...
segunda redacción del mismo hecho -- resolvé eso primero (`darq install --cli <cli> --mcp
id=<clave>`) antes de esperar algo de `"available"`. `revoke` sobre una clave nunca otorgada no es un
error: el JSON trae `"status": "already-revoked"` y sale en `0`. `darq update --cli <cli>`
reaplica las claves ya otorgadas junto con el resto de la selección,
sin que haga falta repetir `mcp grant`.

Si un `install` posterior liga un servidor propio de DARQ (`--mcp id=<clave>`) bajo la misma
...
clave que la persona no nombró en este mismo llamado: si en cambio corre `darq mcp grant` con una
...
### `darq update --cli <id>`: reaplica la selección ya instalada

Usalo cuando la persona pide "actualizar" una instalación que ya existe (por ejemplo, después de que
vos mismo la actualizaste con `darq upgrade`, o simplemente porque pasó tiempo). **No corras
`darq install --cli <id>` a secas para esto:** sobre una instalación que ya tiene una selección
...
```sh
darq update --cli <cli> --dry-run --json
```
...
- Un mensaje que empieza con `"... has bound mcp server(s) ... whose server key was never recorded"`
  ...
  DARQ la guardara, porque adivinar retiraría justo esa atadura. El mensaje mismo trae, listo para
  copiar, el comando `darq install --cli <id> --mcp <id>=<key>` que hace falta correr una vez por
  cada id afectado -- con un `<key>` de relleno en cada uno. **Vos no sabés esa clave.** Conseguila de
  la configuración propia de esa CLI (no es algo que DARQ guarde) o preguntale a la persona; nunca
  inventes un valor para `<key>`. Corré ese `install` una sola vez con la clave real, y a partir de ahí
  `update` vuelve a funcionar sin flags. `darq doctor --json` también imprime este mismo comando,
  bajo `mcp_bound_unknown_keys`, si preferís detectar el caso antes de intentar `update`.

### `darq upgrade`: reemplaza el binario de `darq`
...
`"restart_required": true`. Ese último campo no es adorno: **el proceso de `darq` que hizo el
...
que corras: decile explícitamente que tiene que reiniciar DARQ (cerrar y volver a invocarlo) para
...
- `"... is not writable by this process; upgrade refuses to download anything it could not then
  ...
  `darq.sha256`, y copiar el archivo a mano, con `root`/`sudo` si hace falta). No intentes escalar
  privilegios vos mismo; pasale ese comando a la persona.
- `"darq is not running from an installed executable -- ..."` -- estás corriendo desde un checkout
...
No fuerces colisiones: DARQ las informa y preserva los archivos o claves existentes. Después de
instalar, revisá `darq doctor` y el journal en
`$XDG_DATA_HOME/darq/journal-v4.json` (o `~/.local/share/darq/journal-v4.json`
...
Si hace falta deshacer algo, usá únicamente `darq restore` (vuelve a la generación anterior) o
`darq uninstall --cli <cli>` (retira sólo lo que el journal reclama como propio). No borres
...
alcance exacto del aviso `overwritten`, sus dos huecos y la ventana de `darq restore`.
