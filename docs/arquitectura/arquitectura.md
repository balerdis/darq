...
| `may_delegate_to` no se valida contra los agentes que el producto embarca, así que un nombre mal escrito renderiza un permiso muerto en silencio. Lo declaran ocho agentes; el cargador lo pasa por `_names` y nadie lo mira después. `render.py` escribe `granted["task"] = {"*": "deny", **{name: "allow" ...}}`, así que un `pegasus-explorar` en vez de `pegasus-explorer` produce una clave `allow` que el runtime nunca consulta: el descriptor carga, el agente instala, el resto de la lista funciona, y el agente puede delegar en un destino menos de los que su autor escribió. No hay archivo donde la concesión faltante esté ausente — sólo una lista un nombre más corta de lo que se quiso. Es hermana de la fila de `witheld_tools` resuelta más abajo, y de la misma familia que el caso de `apply_patch`: el producto declara algo correcto y el permiso no llega. Lo cubre un solo test, `test_the_orchestrator_delegates_to_nothing_that_does_not_exist`, y sólo para el orquestador: los otros siete no tienen nada | **Nada técnico, y eso es lo llamativo: el chequeo ya existe para el campo hermano.** `_require_reaches_known_agents` corre en cada `load()` y valida que cada nombre de `reaches` sea un agente embarcado, con un docstring que argumenta exactamente este defecto — «un nombre mal escrito cuesta un destinatario, en silencio». Lo que falta es decidir cuán estricto es el chequeo equivalente: si basta con exigir que el nombre sea un agente embarcado, si además tiene que ser `subagent` —delegar en un `primary` no es lo mismo que un typo—, y si se rechazan los ciclos, que hoy sólo se acotan de costado por el test que impide que un implementador alcance a `sdd-verify` | Al escribir o editar un descriptor, comparar cada nombre de `may_delegate_to` contra los archivos de `content/agents/` antes de confiar en él. Y si un agente no lanza un sub-agente que su descriptor nombra, sospechar del nombre antes que del runtime |
| La pantalla de elección de servidores MCP de la TUI se describe a sí misma como el default seguro y no lo es para una instalación con servidores **atados**. `McpSelectionScreen.chosen` arranca con lo que el journal registra como **instalado** (`tui/navigator.py`), y su docstring afirma que dejar cada fila como se la encontró e ir a Continue «reproduce el estado actual de la máquina en vez de retirar todo en silencio». Un servidor atado —concedido contra una clave que administra la persona, `cbm=codebase-memory-mcp`— está concedido pero no instalado, así que aparece **sin tildar**, y la pantalla no tiene cómo expresar la atadura: alternar una fila produce el nombre pelado, que significa «que Pegasus lo obtenga y lo administre». Continuar sin tocar nada lo deja sin pedir. Medido en una instalación real con `engram` y `cbm` atados: el plan de la TUI retira los dos artefactos de convención **y reescribe 30 cuerpos de agente** para sacarles las instrucciones de esos servidores —33 actualizaciones contra las 3 del mismo install nombrando las ataduras—. No se pierden dos archivos: se le saca el protocolo de memoria persistente a todos los agentes de una | Una decisión sobre cómo se representa un servidor atado en esa pantalla, que es de diseño y no de código: tildado y mostrando su atadura, de modo que destildarlo sea una remoción deliberada; o fuera de la pantalla y preservado, de modo que la pantalla gobierne sólo lo que Pegasus administra y su promesa se vuelva cierta. La primera es más honesta —un servidor atado **es** parte de la instalación— y obliga a que la pantalla lleve la clave al lado del nombre en vez de sólo un booleano | Instalar desde la línea de comandos nombrando cada atadura: `install --cli opencode --mcp context7 --mcp playwright --mcp engram=engram --mcp cbm=codebase-memory-mcp`. Y leer el preview siempre: la sección «Would retire, no longer asked for» es lo único que hoy avisa, y avisa sólo si alguien la lee |
