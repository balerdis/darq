...
| Punto de entrada | Un solo archivo ejecutable (`zipapp` con shebang y bit ejecutable), instalado en `~/.local/bin/darq`. Sin venv privado. |
...
| `bin/darq:390` | `Path(target["opencode_root" if entry["client"] == "opencode" else "claude_root"])` | Solo existen dos raíces posibles, cableadas |
...
You are DARQ's sole readiness authority for executable and configuration changes.
...
```
...
| `agent: darq-orchestrator` | Agente propio de DARQ | `runs_as: orchestrator` |
...
El adapter de OpenCode mapea `orchestrator` a `darq-orchestrator`, `planner` a su agente nativo `plan`, `isolated` a `subtask: true`, y `may_delegate_to` a su bloque `permission`. Otro CLI usa sus propios nombres sin que el descriptor cambie.
...
No hay dato en el journal que las separe. Ninguna de las dos es una remoción, así que el desinstalador las reporta como un segundo resultado: **`unaccounted`**. `removed` es una afirmación sobre algo que DARQ hizo, y acá no sería cierta.
...
Cada códec debe garantizar dos cosas: **serialización canónica** (mismo valor, mismos bytes, para que las huellas sean estables) y **preservación de lo ajeno** (las claves que DARQ no escribió sobreviven intactas al ciclo de lectura y escritura).
...
El journal es lo que hace a DARQ aditivo: registra qué creó, para poder retirarse sin llevarse trabajo ajeno.
...
| Una instalación global | Instalaciones por CLI | Se puede tener DARQ en dos CLIs con ciclos de vida independientes |
...
```
~/.local/share/darq/journal-v4.json     # 0600, en un directorio creado 0700
```
...
- [ ] Un `link` nunca se borra: DARQ no es dueño de dependencias preexistentes
...
| La voz, en sus ocho secciones: `Rules`, `Personality`, `Language`, `Speech patterns`, `Tone`, `Philosophy`, `Expertise`, `Behavior` | `agents/arquitecto-darq.md` | Ese agente |

El criterio no es "reglas arriba, estilo abajo": es a quién obliga cada cosa. La mayoría de las reglas de trabajo — no agregar atribución de herramientas a un commit, verificar antes de afirmar — tienen que obligar a cualquier agente, no solo al que casualmente cargaba ese archivo como prompt. Pero una regla sobre cómo trabaja una voz en particular es de esa voz: `Work out loud or not at all` vive en `arquitecto-darq.md` a propósito, porque como regla global sería falsa — los agentes implementadores escriben en silencio para satisfacer una spec, y eso es exactamente lo que se les pide. El ejemplo anterior acá era `Never build after changes`, que vivía en ese mismo archivo por la misma razón antes de que la reconversión de la voz lo eliminara (ver `Deudas resueltas`): la skill `sdd-verify` exige correr el build y registrar su hash, así que una prohibición global de compilar habría contradicho algo que embarcamos.
...
darq-orchestrator         arquitecto-darq
```
...
```
Where would you like to install DARQ?
...
La TUI no puede hacer nada que los flags no puedan. Esta regla protege `INSTALL_BY_AGENT.md`, que es el diferencial de DARQ: instalación conducida por un agente.

```bash
darq                                          # TUI
darq install --cli opencode --mcp context7 --mcp engram
darq models set --cli opencode --assign sdd-apply=anthropic/claude-sonnet-5 --effort sdd-apply=high
darq models unset --cli opencode --agent sdd-apply
darq uninstall --cli opencode
darq doctor --json
```
...
- `upgrade` es el único que no lleva `--cli`, porque no toca la instalación de ningún CLI: reemplaza el propio binario `darq` en ejecución por el último publicado, verificando el checksum antes de un único rename atómico.
...
- **Si igual no se puede registrar, se retira lo instalado.** Y se informa lo que no pudo volver atrás: DARQ posee claves dentro de un archivo de configuración, nunca el archivo, así que uno que tuvo que crear sobrevive vacío. Inofensivo, pero afirmar un deshecho limpio sería una mentira chica en el único reporte que alguien lee cuando algo ya salió mal.
...
| Descriptor de contenido | `darq/content-descriptor/v1` | Forma de skills, agentes, comandos, prompts, MCPs, políticas |
| Catálogo de artefactos | `pegasus/artifact-catalog/v4` | Salida generada del render, con huellas |
| Journal de ownership | `pegasus-harness/journal/v4` | Estado de instalación |
...
| 6 | TUI: configuración de modelos | Asignar y quitar modelo por agente, con la preferencia guardada en el estado propio de DARQ: la caminata de cuatro pasos (`ModelsScreen`) llama a `cli.models_set`/`cli.models_unset`, que escriben en el store real de asignaciones | Entregada |
...
| `npm` | Materializa el paquete pineado en el directorio propio de DARQ | La cadena del registry: hash de integridad del tarball y, cuando existe, atestación de provenance | Ninguna |
...
> Una asignación de modelo no es una mutación de nuestro artefacto. Es una preferencia que vive en el estado propio de DARQ y participa del render.
...
Y `FileJournalStore.load` hace `if not exists(): return empty()`. Un journal ilegible se convierte en un journal vacío, y todo lo que sigue procede como si DARQ nunca hubiera instalado nada. `ensure_writable`, que corre antes, sólo chequea privilegios y no toca disco, así que no lo atrapa.
...
8b materializa dependencias en el directorio propio de DARQ y las mete al journal como artefactos. Es la unidad que más superficie de archivo nueva agrega, y toda esa superficie pasa por los mismos trece sitios. Arreglar el puerto después significaría auditarla dos veces.
...
`--mcp ID=CLAVE` le dice a DARQ que lo que él llama `cbm` es un servidor que la instalación ya corre bajo otra clave. Concede sus herramientas y embarca su convención; **no obtiene el binario, no escribe la entrada de configuración, no toca la versión**. Existe porque la alternativa era peor: instalar el servidor de DARQ al lado del que la persona administra deja dos versiones sobre un mismo store.
...
`--mcp ID=CLAVE` es para un servidor que DARQ conoce: tiene descriptor, y por lo tanto convención y referencia fuerte. Falta el caso de uno que DARQ nunca vio -- Jira, Figma, cualquier MCP que la persona instaló y administra por su cuenta. Para ese caso no hay descriptor donde declararlo: un `reaches` exigiría un descriptor y una convención que no existen, y forzarlo a esa forma haría que `_require_mcp_convention_referenced` se niegue con razón, porque ningún cuerpo de agente referencia jamás un servidor del que nadie escribió una línea.

`Install.granted_mcp` y `Agent.granted_mcp` son el campo aparte que resuelve esto: `darq mcp grant --cli <id> <clave>` la agrega, y `content.grant_mcp` la aplica **igual a todos los agentes**, nunca por agente -- la decisión explícita fue que agregar un MCP más no debía volverse una tarea tediosa de tocar archivo por archivo. El render escribe `<clave>*` en `tools` y `permission`, exactamente como el comodín de un servidor propio, pero después del comodín de `optional_mcp` y antes de `denied_mcp_tools`: si un servidor propio de DARQ ya negó una herramienta puntual a un agente, un grant del usuario nunca puede reabrirla.

Se rechaza una clave que coincide con un id que DARQ ya embarca, o con una que ya está atada: conceder ahí de nuevo, a todos los agentes por igual, borraría el recorte por agente que esa atadura ya tenía. `mcp grant` además se niega a otorgar una clave que la configuración propia de la CLI no declara -- la misma lógica de `_require_mcp_convention_referenced`, aplicada donde no hay convención que verificar: un error de tipeo no debe traducirse en un permiso que nadie nota que falta.
...
de descarga — una sola regla, no dos que puedan divergir. No hay default a `darq.__version__`:
...
que ya está actualizado. Para el propio `identity.json` de DARQ, un test de versión mantiene
`pyproject.toml`, `darq.__version__` e `identity.version` iguales entre sí; esa igualdad de tres
vías es una propiedad sólo de la distribución propia de DARQ, nunca una regla general.
...
la palabra "DARQ" aparece legítimamente en decenas de docstrings y comentarios; sólo un literal
...
comandos renderizados seguían apuntando a `darq-orchestrator`. La corrección hace que `render.command`
...
DARQ.** Es la única forma en que "una distribución no puede olvidarse de dar su propia identidad"
...
DARQ, y este cambio no los vuelve seguros para dos productos a la vez: la segunda instalación
...
|---|---|---|
| Tres rutas existen: una pregunta va a `darq-explorer`, un check a `darq-verifier` y un cambio chico ya decidido a `darq-implementer`, o lo hace el orquestador dentro de su `Direct Work Threshold` | `src/darq/content/agents/darq-orchestrator.md:33-37` (*"Is this SDD at all?"*); los tres especialistas sin fase son de `009106d` | La consulta y L0 no son nuevas; lo nuevo es FTD, su record y la escalera explícita que lo rodea |
...
| El `Direct Work Threshold` decide cómo se ejecuta, no qué flujo se sigue | `darq-orchestrator.md:15-31` | Se conserva tal cual, y se aplica después de elegir la ruta |
...
| `arquitecto-darq` no tiene techo de palabras (665 hoy) y no menciona `implementation-craft.md` ni Strict TDD | `src/darq/content/agents/arquitecto-darq.md`, leído entero | Gana techo propio en el slice (a) y la resolución de Strict TDD en el (d) |
...
**El "agent marker" de Strict TDD no está definido en ningún lado.** Sólo lo nombra `src/darq/content/skills/sdd-init/SKILL.md:54,62`, como primer escalón para resolver si el modo está activo; ningún archivo dice qué es ni dónde se escribe.
...
Las respuestas se reducen a **cuatro routing facts**, que son a la vez el contrato entre `darq-explorer` y el orquestador y el vocabulario del corpus de tests:
...
**Nombres, y la consulta sin número.** Los niveles se llaman L0, FTD y SDD. La consulta no lleva número ni etiqueta propia: alcanza con nombrarla en el orden de `flow-applicability.md`, porque hoy nada la identifica y nada lo necesita. **FTD se activa sólo por routing en v7**: `src/darq/content/commands/` no tiene infraestructura de comandos para rutas fuera de SDD, y un comando puede sumarse después sin tocar el contrato de facts.
...
|---|---|---|---|
| Pistas | El cuerpo de `darq-orchestrator.md` y el de `arquitecto-darq.md` | ¿Información o intención de cambio?, ¿pidió SDD, spec o plan?, y que el resto requiere clasificar | Siempre, como hoy |
| Decidir | `src/darq/content/skills/_shared/flow-applicability.md`, renombre de `sdd-applicability.md` | Las preguntas de cada frontera, los cuatro facts, el orden de decisión y las promociones, con lo que el archivo viejo ya tenía de valor —casos ambiguos, crecimiento a mitad de camino, fail-open— generalizado a las cuatro rutas | Sólo cuando la ruta no es obvia |
| Ejecutar | FTD: `src/darq/content/skills/_shared/ftd-procedure.md`, nuevo. SDD: el preflight y los gates que ya existen. L0: casi nada | El record, las reglas de evidencia y el cierre de FTD | Sólo al recorrer esa ruta |
...
|---|---|---|
| `darq-orchestrator.md` | 1409 | 1420, sin cambio: la sección se reescribe dentro del margen |
| `arquitecto-darq.md` | 665 | Propio y nuevo, en el slice (a) |
...
| Agente | En v7 |
|---|---|
| `darq-orchestrator` | Enruta, coordina y hace él mismo los chequeos y la investigación acotada que necesita para decidir y preparar delegaciones; no es un delegador puro. Decide la ruta primero y aplica después el `Direct Work Threshold`, `may_delegate_to`, las capacidades del target y las reglas de paralelismo. No hay pipeline fijo de subagentes para L0 ni para FTD: cada handoff se justifica y nada se delega por ceremonia |
| `arquitecto-darq` | **Aplica a todas las rutas salvo SDD.** Como escribe, arma él mismo el record FTD; si un trabajo suyo tiene que pasar a SDD, lo dice y deriva al orquestador, sin correr el ciclo. Su `may_delegate_to: [darq-general]` (`arquitecto-darq.md:6`) no cambia |
| `darq-implementer` | El gemelo operativo de `sdd-apply` fuera de SDD: ejecuta L0 y FTD sin heredar preflight, artifacts ni gates exclusivos de SDD |
| `darq-verifier` | Sigue devolviendo sólo evidencia, nunca un veredicto de readiness |
| `darq-general` | El worker sin identidad de fase, usable en L0, en la investigación previa y en FTD |
| `darq-explorer` | Investigación read-only sin artifact de fase; en v7, además, devuelve routing facts cuando se le piden |
...
**Readiness fuera de SDD: la opción (a).** El readiness de L0 y FTD lo declara quien pidió el trabajo —el orquestador, o `arquitecto-darq` en su propia sesión— leyendo la evidencia, que es lo que el verifier ya dice de sí mismo: *"leave the conclusion to whoever asked for it"* (`darq-verifier.md:17`). En FTD se ve en el record: un ítem está hecho sólo si tiene asociada una observación concreta. La separación de `009106d` se conserva tal cual: `darq-implementer` y `darq-general` pueden llamar a `darq-verifier` (la línea 6 de cada uno), y justamente por eso el verifier no firma:

> **Nadie que un escritor pueda alcanzar tiene autoridad para firmar.**

**Cuando quien firma también escribió** —un L0 inline del orquestador, o todo lo de `arquitecto-darq`—, nadie declara "listo" sin una observación registrada, y quien firma algo que escribió lo dice explícitamente en vez de presentarlo como verificación independiente. Es la extensión del *"Close the loop you open"* de `arquitecto-darq.md:19`, que ya cubría casi todo.
...
**Un escritor por vez: el record lo mantiene quien coordina el FTD**, el orquestador o `arquitecto-darq`. Los subagentes devuelven evidencia y él la incorpora.
...
El mismo historial dejó tres reglas más, propias de un flujo de trabajo entre repositorios y no del producto, y no entran.

FTD tampoco impone branch automático, prohibición de worktrees, el pipeline SDD completo ni `sdd-verify`: delivery, Git y aislamiento siguen la policy existente de DARQ.

### Strict TDD fuera de SDD

Hoy `_shared/implementation-craft.md:28` tiene el gate *"when Strict TDD Mode is active"* y `darq-implementer` lo lee, pero ningún archivo le dice al implementer ni a `arquitecto-darq` cómo saber si está activo: la resolución vive sólo en `sdd-init/SKILL.md:54-62`. Con FTD como carril principal, sin esto el carril principal saldría sin TDD. El slice (d) lo resuelve así:
...
4. **Se resuelve una vez por sesión y viaja en el brief.** El orquestador, o `arquitecto-darq`, la resuelve como ya resuelve las respuestas del preflight; si el brief no la trae, el implementer la resuelve con la misma regla.
...
Además del corpus: los tests de techo, con su gemelo, para `flow-applicability.md` y `arquitecto-darq.md`; un test de lazy-load que fija que ningún cuerpo always-on incorpora el texto de `flow-applicability.md` ni de `ftd-procedure.md`, y que el procedimiento sólo se nombra en la salida de la ruta FTD de `flow-applicability.md`; y el test de punta a punta del renombre.
...
**Rollback.** v7 es contenido y tests, sin wire formats nuevos: volver atrás es reinstalar con el binario de una versión anterior, cuyo render ya no produce `flow-applicability.md` ni `ftd-procedure.md`, así que el mismo retiro por dirección los borra y `sdd-applicability.md` vuelve a escribirse. Los records que ya estén en `docs/ftd/` son archivos del proyecto, no de DARQ: el journal no los reclama y ningún rollback los toca.
...
|---|---|---|
| 1 | `src/darq/content/skills/_shared/sdd-applicability.md` no existe y `flow-applicability.md` sí | `test -e` sobre las dos rutas |
...
| 6 | El cuerpo del orquestador sigue en 1420 palabras o menos; `arquitecto-darq.md` y `flow-applicability.md` tienen techo propio, cada uno con su gemelo contra el re-envolver | `OrchestratorStaysSmallTest` y sus equivalentes nuevos |
...
| 11 | La regla de lectura de Strict TDD vive en un único archivo; `sdd-init`, `sdd-apply`, `sdd-verify`, `arquitecto-darq` y `darq-implementer` la referencian en vez de repetirla, el preflight y el procedimiento FTD la pasan en el brief, y `N/A: no behavior changed` figura como resultado válido del gate | Tests de contenido del slice (d) |
| 12 | `darq-verifier` sigue sin emitir veredicto y ningún `may_delegate_to` cambia | `tests/test_phase_less_specialists.py` y el diff del frontmatter de los agentes |
...
|---|---|---|
| (a) Escalera y routing | `flow-applicability.md` —renombre y ampliación, con techo—; pistas mínimas en el orquestador, texto por texto dentro de su techo, y en `arquitecto-darq`, que gana techo propio; routing facts en `darq-explorer`; el corpus y sus tests; y el test de punta a punta del renombre | **Cerrado.** a1–a4 en `c7894ef`: corpus de 48 casos con citas que coinciden con la ruta, test de punta a punta del renombre en los dos adapters, `flow-applicability.md` (1026 palabras) y orquestador en 1398; suite 2938 contra la base de 2892. a5–a6 en `72c363d`: el puntero de `arquitecto-darq` a la escalera con su fail-open y la derivación de SDD al orquestador, y los techos de `flow-applicability.md` (1061) y `arquitecto-darq.md` (783), medidos más las reservas de b5, c3 y d5; suite 2945. a7: `darq-explorer` devuelve los routing facts cuando el brief los pide y no elige la ruta; el chequeo es de mundo cerrado, porque tres chequeos de negación léxica se dejaron engañar en este slice (469 de 516 palabras). Cierre: catálogo en (98, 27), el nombre viejo solo en la constante del test del renombre, el renombre retirado en los dos adapters, orquestador en 1398; suite 2954 |
...
| b3 | El procedimiento | `src/darq/content/skills/_shared/ftd-procedure.md`, nuevo, en inglés | b1 y b2 | `unittest tests.test_ftd_procedure tests.test_catalog tests.test_skill_references tests.test_adapter_reference_integrity tests.test_engram_convention` verde; `EngramProjectArgumentTest` impide templar el argumento de proyecto | b1, b2 |
...
| # | Tarea | Archivos | Test primero (rojo) | Evidencia de cierre | Depende de |
|---|---|---|---|---|---|
| c1 | Tests de readiness fuera de SDD | `tests/test_orchestrator_routing.py` (`ReadinessOutsideSddTest`), `tests/test_persona_split.py` (`PersonaTest`), `tests/test_phase_less_specialists.py` (`WhatEachOneReturnsTest`) | El orquestador dice que en L0 y FTD el readiness lo declara quien pidió, leyendo evidencia observada, y que quien firma lo que escribió lo dice; *"until `sdd-verify` has spoken"* queda acotado a SDD en su misma oración; `arquitecto-darq` dice lo mismo junto a *"Close the loop you open"*; `darq-implementer` y `darq-general` dicen que el record lo lleva quien coordina y que ellos devuelven evidencia. Rojo: ninguna frase está | Los tres módulos, rojos antes de c2-c4 | b3 |
| c2 | Orquestador | `darq-orchestrator.md:98-100`, la viñeta *"Warmth is never a readiness claim"*, texto por texto | c1 | `OrchestratorStaysSmallTest` ≤ 1420 y `ReadinessAuthorityScopeTest` (`tests/test_readiness_authority_scope.py`) verdes: la frase nueva no reclama autoridad, porque ese guardia sólo admite la afirmación en `sdd-verify` y en `darq-orchestrator.md:47` | c1, a3 |
| c3 | `arquitecto-darq` | `arquitecto-darq.md:19`, y la regla de que arma él mismo el record, al que llega por `flow-applicability.md` | c1 | `ArquitectoDarqStaysSmallTest` y `PersonaTest` verdes | c1, a6 |
| c4 | Implementer, general y verifier | `darq-implementer.md` y `darq-general.md`, `## Result identity`; `darq-verifier.md` sin cambio | c1 | `unittest tests.test_phase_less_specialists tests.test_general_fan_out_checkpoint` verde, con `BODY_WORD_CEILING` = 516 y `GENERAL_WORD_CEILING` = 556; `git diff --stat -- src/darq/content/agents/darq-verifier.md` vacío; `WhatEachOneReturnsTest` y `RenderedPermissionTest` siguen probando que el verifier no firma ni escribe | c1 |

**Cierre propio de (c):** `darq-verifier.md` sin diff, y `ReadinessAuthorityScopeTest` y `WhatEachOneReturnsTest` verdes sin haberlos tocado.
...
| # | Tarea | Archivos | Test primero (rojo) | Evidencia de cierre | Depende de |
|---|---|---|---|---|---|
| d1 | Tests de la regla compartida | `tests/test_craft_extraction.py`: `CraftIsNotRestatedTest` y una `StrictTddResolutionTest` nueva | La regla de lectura vive sólo en `implementation-craft.md` (una frase distintiva, con `occurrences` igual a ese archivo); `sdd-init`, `sdd-apply`, `sdd-verify`, `arquitecto-darq` y `darq-implementer` la señalan en vez de repetirla; las filas de `sdd-init/SKILL.md:54-56` y el paso `:62` ya no resuelven; la puerta de `sdd-verify/SKILL.md:43` usa la grafía del brief; `N/A: no behavior changed` figura como resultado válido del gate; la grafía del marker está escrita como literal en `implementation-craft.md`, y ningún archivo embarcado la trae como línea suelta, así que DARQ nunca activa el modo en nombre de la persona. Rojo: nada de eso está | `unittest tests.test_craft_extraction -v`: rojo | — |
| d2 | La regla y el marker | `src/darq/content/skills/_shared/implementation-craft.md`: una subsección nueva antes de *Strict TDD Hard Gate* (`:28`) | d1 | `unittest tests.test_craft_extraction tests.test_strict_tdd_red_evidence tests.test_phase_less_specialists` verde (`CRAFT_NEEDLE` sigue en un solo archivo) | d1 |
| d3 | `sdd-init` pasa a usar la regla | `src/darq/content/skills/sdd-init/SKILL.md:54-56` y `:62`; sigue escribiendo `strict_tdd` en `openspec/config.yaml` y en `sdd/{project}/testing-capabilities` (`references/init-details.md:38`, `:59`) | d1 | `unittest tests.test_craft_extraction tests.test_sdd_phase_macros` verde | d2 |
| d4 | Los otros dos lectores | `src/darq/content/skills/sdd-apply/SKILL.md:110-139`: primero el valor del brief, después la regla; `src/darq/content/skills/sdd-verify/SKILL.md:43-45` | d1 | `unittest tests.test_craft_extraction tests.test_strict_tdd_red_evidence` verde | d2 |
| d5 | Una vez por sesión, en el brief | `_shared/sdd-session-preflight.md`, junto a `:58`, que ya manda `Artifact store mode` a cada lanzamiento; `_shared/ftd-procedure.md`; `arquitecto-darq.md`, que nombra `implementation-craft.md` y resuelve el modo en su propia sesión. El cuerpo del orquestador no cambia | `StrictTddTravelsInTheBriefTest` en `tests/test_craft_extraction.py`: el preflight y el procedimiento FTD mandan `Strict TDD Mode` en cada brief de implementación, y `arquitecto-darq` nombra `_shared/implementation-craft.md`. Rojo: no está | `unittest tests.test_craft_extraction tests.test_ftd_procedure tests.test_orchestrator_routing` verde, `ArquitectoDarqStaysSmallTest` incluido | d2, b3, a6 |
...
| Dónde vive | En las instrucciones que el CLI anfitrión carga en la sesión, las del proyecto o las de la persona. Nunca en un archivo que escribe DARQ, porque reinstalar lo reemplaza entero. El contenido tampoco puede nombrar el archivo de instrucciones de un CLI concreto (`tests/test_content_core_is_cli_agnostic.py`): nombra la línea, no el archivo |
...
| `arquitecto-darq.md` se edita en (a), (c) y (d) | El techo de a6 se mide con el margen de c3 y d5 escrito en su comentario; si no alcanza, el techo sube a propósito, nunca re-envolviendo líneas |
...
| El texto de hoy contradice lo decidido: `sdd-applicability.md` pone como señales de SDD *"an approach worth arguing, trade-offs"*, *"across several sessions"* y *"a written record has to survive the session"*, y dos casos ambiguos mandan una decisión de diseño a SDD; `darq-orchestrator.md:35` dice que un record que sobrevive a la sesión es SDD | El renombre invierte esas señales en vez de conservarlas, y los tests de a3 fijan su ausencia |
...
|-------|------------------|
| En `tests/test_craft_extraction.py`, las constantes que apuntan a `arquitecto-darq.md` quedaron como `KING_PEGASUS` y `KING_PEGASUS_TDD` después del transporte de v7, mientras `tests/test_persona_split.py` localizó las suyas como `ARQUITECTO_DARQ_*`. Son identificadores internos y no cambian nada de lo que se prueba; el guardián de transporte no los ve porque no tienen guion | No hizo falta tocar código: esas constantes ya se llamaban `ARQUITECTO_DARQ` y `ARQUITECTO_DARQ_TDD` desde el propio transporte (`8e90e0e`), que localizó las `KING` y `KING_TDD` de upstream. `KING_PEGASUS` nunca existió en `tests/test_craft_extraction.py`: es el nombre que upstream usa en `tests/test_orchestrator_routing.py`, y acá esa constante también quedó como `ARQUITECTO_DARQ` desde `74f0aa9`. Medido al cerrarla: ningún `KING_PEGASUS`, `KING` ni `KING_TDD` en `src/`, `tests/` ni `tools/`, y el corpus de vocabulario que llegó después (`tests/test_closed_world_vocabulary.py`) no nombra ninguna de esas constantes, así que tampoco hubo que ajustarlo |
| Este documento nombra en prosa algunos identificadores de wire congelados con la marca de DARQ (`darq/cli-report/v1`, `darq/journal/v4`, `darq-registry-assets/v3`, `darq_installed`), cuando el código conserva, como corresponde, `pegasus/cli-report/v1`, `pegasus-harness/journal/v4`, `pegasus-registry-assets/v3` y `pegasus_installed`. Viene de antes de v7 y es solo documentación | Se devolvieron a su forma congelada, con el código y la sección 6 del runbook como verdad: `pegasus/cli-report/v1` en sus dos menciones (`cli.SCHEMA`), `pegasus-harness/journal/v4` en la tabla de contratos (`journal.SCHEMA`, con el `-harness` que el código escribe), `pegasus-registry-assets/v3` y `pegasus_installed` en sus dos menciones. En la misma pasada, la fila resuelta sobre los nombres de artefactos on-disk, que narra el estado anterior a derivarlos, volvió a nombrar lo que el motor fijaba entonces: `pegasus-AGENTS.md` —que sigue siendo el default congelado de `adapters/opencode/layout.py`—, `pegasus/skill-registry` y «Pegasus». Se buscó con la marca de DARQ cada identificador de la sección 6 y no queda ninguno fuera de esta fila. Ningún token restaurado necesitó exención del guardián de transporte: todos separan la palabra con `/`, `-` o `_` en formas que sus patrones no reconocen, que es lo que el docstring del guardián promete. `darq/content-descriptor/v1`, en la misma tabla, no se tocó: no figura en la sección 6 y ningún código lo emite, así que no es un identificador de wire |
...
|-------|------------------|
| `darq setup` sólo podía reconstruir el venv de una instalación si esa instalación alguna vez corrió desde un checkout, porque esa corrida es la que deja los insumos en `setup-sources/` | DARQ quedó sin dependencias y el punto de entrada pasó a ser un `zipapp` con shebang y bit ejecutable. Sin dependencias, un venv privado no aísla nada; sin necesidad de aislar nada, no hace falta shim que lo arranque. `darq setup`, el venv, el shim y `setup-sources/` se retiraron enteros, y la deuda dejó de tener sobre qué pararse |
...
- [ ] Cada códec serializa de forma canónica y preserva las claves que DARQ no escribió
...
- [ ] Desinstalar deja el sistema sin rastros de DARQ, incluido lo que el usuario modificó sobre una dirección propia; lo único que puede quedar es un ítem de lista que no se pudo identificar
