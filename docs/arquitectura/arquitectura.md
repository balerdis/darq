...
Cada reporte declara su esquema `darq/cli-report/v1`.
...
| Journal de ownership | `darq/journal/v4` | Estado de instalación |
| Asignación de modelo | `pegasus/model-assignment/v1` | Proveedor, modelo y esfuerzo por agente |
| Reporte de la CLI | `darq/cli-report/v1` | Salida JSON de `install`, `uninstall` y `doctor` |
...
`journal-v4.json`, las claves `pegasus_version`/`darq_installed`, etc. — ver más abajo).
...
las claves JSON `pegasus_version`/`darq_installed`, y las variables de entorno
