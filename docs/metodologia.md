...
Una decisión o un trade-off que resolver, un trabajo que cruza sesiones o un record que tiene que durar no llevan a SDD: son FTD. Las fronteras entre rutas están en [flow-applicability.md](../src/darq/content/skills/_shared/flow-applicability.md).
...
El record se versiona con Git por defecto. El FTD que crea `docs/ftd/` avisa una sola vez dónde quedó; si no se quiere subir, se excluye con `.gitignore` o con `.git/info/exclude`, que no se versiona. Con Engram disponible, la ruta del record también queda en memoria; sin Engram, el agente la informa en la conversación. Cómo se arma el record y qué cuenta como evidencia está en [ftd-procedure.md](../src/darq/content/skills/_shared/ftd-procedure.md).
...
| `darq-orchestrator` | Elige la ruta de cada pedido, coordina el trabajo y, en SDD, ordena fases, valida gates y delega el trabajo correcto. | No ejecuta inline las fases que pertenecen a un subagente. |
