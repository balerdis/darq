...
| --- | --- | --- | --- |
| `cbm` | `darq` | CBM | Engram, Playwright, Context7 |
| `engram` | `darq-engram` | Engram | CBM, Playwright, Context7 |
| `playwright` | `darq-playwright` | Playwright | CBM, Engram, Context7 |
| `context7` | `darq-context7` | Context7 | CBM, Engram, Playwright |
| `final` | `darq-final` | Todos | Ninguno |

## Ejecución

El único punto de entrada es `accept-v3-isolated.sh`. `provision-v3-rc-host.sh` no es un flujo de producto: es el emulador de host de laboratorio que el orquestador invoca después del preflight. Root extrae y verifica el RC en el staging privado; después entrega una copia nueva bajo `/var/lib/darq-acceptance/<target>/rc`, cuyos ancestros son root-owned y no admiten escritura del target. El payload permanece `root:<grupo primario del target>` con directorios y `bin/darq` en `0750`, otros archivos en `0640`: el target puede recorrer, leer y ejecutar el payload verificado, pero no reemplazarlo; no es legible globalmente.
...
```sh
sudo install -d -o root -g "$(id -g serg)" -m 0750 /var/lib/darq-acceptance-evidence/v3.1.0-rc.1
sudo -n -u serg -H install -d -m 0700 /var/tmp/darq-v3.1.0-rc.1-aggregate-serg
```
...
sudo ./scripts/accept-v3-isolated.sh \
  --profile cbm \
  --rc-archive /releases/darq-v3.1.0-rc.1.tar.gz \
  --rc-checksum /releases/darq-v3.1.0-rc.1.tar.gz.sha256 \
  --release-manifest /releases/release-manifest.json \
  --staging-dir /var/tmp/darq-v3.1.0-rc.1-cbm \
  --evidence-file /var/lib/darq-acceptance-evidence/v3.1.0-rc.1/cbm.json \
  --evidence-verifier serg \
  --confirm-recreate-user darq
```

Para Playwright, instalá el navegador externamente; DARQ no lo descarga. Se puede pasar `--browser <ruta-absoluta>` al orquestador, que lo propaga al provisionador y al preflight. La ruta debe ser un ejecutable regular, no symlink, root-owned, sin escritura grupal/global y fuera del home objetivo; todos sus ancestros deben cumplir la misma propiedad y permisos. Si se omite, se conserva la detección existente en `~/.cache/ms-playwright/chromium` y `~/.local/bin/chromium`. Una prueba fallida requiere un commit y RC nuevos: nunca se muta un tag.

## Artefactos verificados

- Node `24.15.0`: descarga oficial Linux x64 y SHA-256 `472655581fb851559730c48763e0c9d3bc25975c59d518003fc0849d3e4ba0f6`; queda en `~/.local/darq-acceptance/node`.
...
- [ ] El payload de handoff está bajo `/var/lib/darq-acceptance/<target>/rc`; todos sus ancestros son root-owned sin escritura grupal/global, y el payload es `root:<grupo del target>` (`0750` directorios/entrypoint, `0640` otros archivos), sin lectura global.
