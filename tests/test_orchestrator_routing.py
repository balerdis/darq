...
ARQUITECTO_DARQ = AGENTS / "arquitecto-darq.md"
...
#:
#: In this fork every count below was re-measured on its own tree after the
#: transport, not copied from upstream, and each one came out the same: these
#: files are upstream's word for word apart from the agent names and the
#: persona's title, which have the same word count on both sides.
#:
...
#: `arquitecto-darq.md`: 715 words measured after v7 a5 -- 33 of them front
...
ARQUITECTO_DARQ_WORD_CEILING = 783
...
#: "ship" is left out because `darq-general` uses it for something else
...
ORCHESTRATOR_READINESS_SENTENCES = frozenset(
    {
        ...
        "`sdd-verify` is DARQ's sole authority for declaring that an SDD change is ready to archive.",
...
CEILINGED_BODIES = (
    ...
    (ARQUITECTO_DARQ, ARQUITECTO_DARQ_WORD_CEILING),
)
...
class ArquitectoDarqStaysSmallTest(unittest.TestCase):
    """An always-on body: every word is paid on every turn of the voice."""

    def test_the_teaching_voice_did_not_grow(self):
        words = len(ARQUITECTO_DARQ.read_text(encoding="utf-8").split())
        self.assertLessEqual(words, ARQUITECTO_DARQ_WORD_CEILING, "arquitecto-darq.md grew")
