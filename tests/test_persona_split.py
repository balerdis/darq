...
ARQUITECTO_DARQ_RECORD = "On FTD you keep the record yourself; that file leads you to how it is kept."
...
class PersonaTest(SharedContentRules, unittest.TestCase):
    ...
    def test_readiness_sits_beside_close_the_loop(self):
        self.assertIn(ARQUITECTO_DARQ_READINESS, self.rule("Close the loop you open"))
        self.assertEqual(" ".join(self.text.split()).count(ARQUITECTO_DARQ_READINESS), 1)
    ...
    def test_it_keeps_the_ftd_record_itself_through_the_ladder(self):
        ...
        self.assertIn(ARQUITECTO_DARQ_RECORD, self.rule("{{skills_root}}/_shared/flow-applicability.md"))
        self.assertEqual(" ".join(self.text.split()).count(ARQUITECTO_DARQ_RECORD), 1)
        self.assertNotIn("ftd-procedure.md", self.text)

    def test_no_other_sentence_speaks_of_readiness(self):
        self.assertEqual(sentences_on(self.text, READINESS_STEM), {ARQUITECTO_DARQ_READINESS})

    def test_no_other_sentence_speaks_of_the_record(self):
        self.assertEqual(sentences_on(self.text, RECORD_STEM), {ARQUITECTO_DARQ_RECORD})
...
class ArquitectoDarqRoutesTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.agent = next(a for a in content_module.load().agents if a.name == "arquitecto-darq")
    ...
    def test_work_that_needs_sdd_is_handed_to_the_orchestrator_not_run_here(self):
        rules = self.bullets_naming("SDD", "`darq-orchestrator`")
        ...
        handed_over = re.compile(r"\bhand(?:s|ed)?\s+(?:it\s+)?over\s+to\s+`darq-orchestrator`")
    ...
    def test_handing_sdd_over_adds_no_delegation_target(self):
        ...
        delegating to it: the voice still delegates to `darq-general` only."""
        self.assertEqual(self.agent.may_delegate_to, ("darq-general",))
