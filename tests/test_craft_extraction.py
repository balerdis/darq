...
#: peeled off. DARQ never switches the mode on or off for the person, so no
...
ARQUITECTO_DARQ = CONTENT / "agents" / "arquitecto-darq.md"
...
TDD_UNITS = {
    ...
    ARQUITECTO_DARQ: frozenset({ARQUITECTO_DARQ_TDD}),
...
class CraftIsNotRestatedTest(unittest.TestCase):
    ...
    def test_the_implementer_reaches_the_rule_through_its_craft_file(self):
        implementer = (CONTENT / "agents" / "darq-implementer.md").read_text(encoding="utf-8")
...
class StrictTddResolutionTest(unittest.TestCase):
    ...
    def test_no_shipped_file_carries_a_marker_line(self):
        """DARQ never decides the mode for the person: a bare marker line
    ...
    def test_a_brief_without_a_recognized_spelling_falls_back_to_the_rule(self):
        ...
        from an orchestrator outside DARQ means the rule decides, never a
...
class StrictTddTravelsInTheBriefTest(unittest.TestCase):
    ...
    def test_arquitecto_darq_resolves_the_mode_in_its_own_session_through_the_craft(self):
        rules = ARQUITECTO_DARQ.read_text(encoding="utf-8").split("\n## Rules\n", 1)[1].split("\n## ", 1)[0]
        self.assertEqual(" ".join(rules.split()).count(ARQUITECTO_DARQ_TDD), 1)
        self.assertIn("{{skills_root}}/_shared/implementation-craft.md", ARQUITECTO_DARQ_TDD)
        self.assertNotIn("ftd-procedure.md", ARQUITECTO_DARQ.read_text(encoding="utf-8"))
